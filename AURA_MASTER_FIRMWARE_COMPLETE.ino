...
const String OPENROUTER_KEY = "YOUR_OPENROUTER_API_KEY";
const char* OPENROUTER_MODEL = "google/gemini-flash-1.5";

// Target Coordinates: Blue Location Point on Nehru Street, Sriperumbudur
...
// ----------------------------------------------------------------------------
// TRUE MATRIX NOISE FLOORS & DEBOUNCERS
// ----------------------------------------------------------------------------
...
volatile float targetAzimuthDeg = 42.0;
String targetAzimuthVector = "042° NE";
...
String humanScentLabel = "CLEAN AMBIENT AIR (400 PPM)";
...
// ============================================================================
// SCIENTIFIC MULTI-SENSOR BIO-PHYSICAL CONSENSUS ALGORITHM
// ============================================================================
void processSpatialIntelligence() {
  bool motionActive = (rawRadar == 1);
  bool acousticActive = (micEnergy > 10.0f); 
...
// ============================================================================
// EMBEDDED WEB DASHBOARD (SERVED AT http://<NODE_IP>/)
// ============================================================================
...
<script>
  ...
  try{
    ...
    document.getElementById('scentLabel').innerText=d.human_scent_label||'CLEAN AMBIENT AIR';
...
// ============================================================================
// DIRECT HARDWARE REGISTER MPU-6050 ENGINE (BYPASSES WHO_AM_I BUGS)
// ============================================================================
...
void loop() {
  ...
  if (millis() - lastDspCycle >= 4) {
    ...
    // RCWL-0516 RADAR DETECTION (Instant Catch & 2.5s Latch)
    ...
    // ========================================================================
    // ULTRA-HIGH SENSITIVITY PIEZOELECTRIC SEISMIC DSP (Burst Sampling)
    // ========================================================================
    // Captures microsecond transient shockwaves (taps/knocks) across 150us burst
    int piezoMin = 4095;
    int piezoMax = 0;
    for (int i = 0; i < 32; i++) {
      int p = analogRead(PIN_PIEZO);
      if (p < piezoMin) piezoMin = p;
      if (p > piezoMax) piezoMax = p;
      delayMicroseconds(4);
    }
    int peakToPeakPiezo = piezoMax - piezoMin;
    rawPiezo = piezoMax;

    // Dynamic seismic baseline & envelope tracking
    piezoBaseline = (piezoBaseline * 0.98f) + ((float)piezoMin * 0.02f);
    float piezoInstantDelta = (float)peakToPeakPiezo;

    // High-gain impulse boost for maximum sensitivity to light debris taps
    float scaledImpulse = piezoInstantDelta * 22.0f;
    if (scaledImpulse > (float)piezoPeakEnvelope) {
      piezoPeakEnvelope = scaledImpulse;
    } else {
      piezoPeakEnvelope = (float)piezoPeakEnvelope * 0.88f;
    }

    // Precise tap event latching (min 70ms debounce, 2.5s multi-tap grouping)
    if (scaledImpulse > 12.0f) {
      unsigned long now = millis();
      if (now - lastTapTimestamp > 70 && now - lastTapTimestamp < 2500) {
        tapCountWindow++;
        lastTapTimestamp = now;
      } else if (now - lastTapTimestamp >= 2500) {
        tapCountWindow = 1;
        lastTapTimestamp = now;
      }
    }
    ...
    // ========================================================================
    // ULTRA-HIGH SENSITIVITY MAX9814 ACOUSTIC SENSOR DSP (Full Waveform Cycle)
    // ========================================================================
    // 96 samples over ~1.6ms spans the fundamental wavelength of human voice & distress calls
    int micMin = 4095;
    int micMax = 0;
    for (int i = 0; i < 96; i++) {
    ...
    // Adaptive noise floor cancellation
    micNoiseFloor = (micNoiseFloor * 0.995f) + ((float)currentRawMic * 0.005f);
    float trueAudio = (float)currentRawMic - micNoiseFloor;
    if (trueAudio < 0) trueAudio = 0;

    // Dynamic AGC amplification (x2.8 multiplier for distant whispers & tapping)
    float amplifiedAudio = trueAudio * 2.8f;

    if (amplifiedAudio > 5.0f) {
      micEnergy = ((float)micEnergy * 0.15f) + (amplifiedAudio * 0.85f);
    } else {
      micEnergy = (float)micEnergy * 0.75f;
      if (micEnergy < 1.0f) {
        micEnergy = 0.0f;
      }
    }
  ...
  if (millis() - lastSlowCycle >= 200) {
    ...
    if (millis() - lastTapTimestamp > 6000) {
        tapCountWindow = 0;
    }
